...
One `storage_paths` entity is created per Recipient (Paperless has no
`{{ Recipient }}` placeholder, so the recipient is baked into the path at
creation time). The template, with `<Recipient>` substituted with the actual
recipient name:
